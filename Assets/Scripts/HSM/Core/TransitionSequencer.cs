...
public class TransitionSequencer {
    ...
    static List<PhaseStep> GatherPhaseSteps(List<State> chain, bool deactivate) {
        ...
        for (int i = 0; i < chain.Count; i++) {
            ...
            for (int j = 0; j < acts.Count; j++){
                ...
                // Only include if trying to deactivate and is active, or trying to activate and is inactive
    ...
    public void Tick(float deltaTime) {
        if (sequencer != null) {
            if (sequencer.Update()) { // returns true on phase done
                if (nextPhase != null) {
                    var n = nextPhase;
                    nextPhase = null;
                    n();
    ...
    {
        if (sequencer != null) return; // same gate as Tick
